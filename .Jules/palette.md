...
**Learning:** When building custom filter interfaces (like a 'Clear Filters' button) alongside DataTables, the native global search box is often forgotten, leading to a stranded filtered state where users click 'Clear' but the search term remains active.
**Action:** Always synchronize custom filter reset actions with the DataTables search state by checking `table.search()` for visibility toggles, binding to the `search.dt` event, and explicitly clearing it with `table.search('').draw()`.
