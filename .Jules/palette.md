...
## 2026-04-24 - Escape Shortcut for Contextual Actions
**Learning:** Contextual actions like "Clear Filters" that appear dynamically are easily missed by keyboard users who prefer not to tab through the interface backwards. Binding `Escape` to reset the view state provides an expected, native-feeling interaction, but it must be careful not to override native element behaviors (like closing a `<select>` dropdown).
**Action:** When implementing temporary contextual actions (like a 'Clear Filters' button), bind the `Escape` key to trigger the reset. Ensure the listener ignores native elements like `select` (`!$(e.target).is('select')`) to preserve their native Escape behavior, blur the active element afterward, and pair the shortcut with an explicit visual hint (e.g., appending '(Esc)') in the UI.
