...
## 2026-04-24 - Comprehensive Filter Reset Logic
**Learning:** When implementing "Clear Filters" logic alongside DataTables, relying solely on custom `<select>` dropdowns leaves the global search state untouched and unresponsive. Users who typed a search term and clicked "Clear Filters" would find their text persists and the table remains filtered.
**Action:** Always listen to the global DataTables search event (`table.on('search.dt')`) to toggle filter reset buttons, and explicitly clear it (`table.search('').draw()`) alongside other custom filter states.
