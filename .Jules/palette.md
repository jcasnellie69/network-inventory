...
## 2026-04-24 - Escape Key for Contextual Resets
**Learning:** When a UI presents temporary contextual actions (like a "Clear Filters" button that appears conditionally), allowing keyboard users to dismiss or trigger that action via the `Escape` key significantly improves navigation speed.
**Action:** When implementing contextual reset buttons or overlays, bind the `Escape` key to trigger the reset/dismissal and explicitly communicate this shortcut to users within the button text (e.g., "Clear Filters (Esc)").
