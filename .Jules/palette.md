...
**Learning:** Adding a global keyboard shortcut (like `/` for search) is great for power users, but invisible to regular users. Users often won't read documentation for internal tools.
**Action:** When adding global keyboard shortcuts, always provide an inline visual hint, such as updating the input's placeholder text (e.g., `Search (Press '/')...`) to maximize discoverability without adding UI clutter.
