...
## 2026-05-06 - Fixing Clear Filters & DataTables Search Interaction
**Learning:** When managing external custom filter controls in conjunction with jQuery DataTables, binding only to `<select>` dropdown changes is insufficient if the native DataTables text search box is also used. The search state must be explicitly cleared using `table.search('').draw()` alongside the dropdowns to reset the view entirely. Additionally, it's vital to bind to the `search.dt` event to reflect the state correctly in the "Clear Filters" UI when a user manually searches, and to scope the filter dropdown listeners (e.g., `.filters select`) so they don't accidentally intercept DataTables' own generated internal elements (like the "length menu" select).
**Action:** Always bind custom "Clear Filters" controls to both global text search elements (`table.search()`) and external category dropdowns to avoid partially hidden data. Listen to the custom `search.dt` event to accurately toggle visibility of clear actions based on total filter state.
